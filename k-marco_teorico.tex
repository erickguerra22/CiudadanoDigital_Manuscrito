...
Los modelos de lenguaje de gran escala (\textit{Large Language Models} o LLMs)
son sistemas de inteligencia artificial entrenados con enormes volúmenes de
texto para comprender y generar lenguaje natural. Estos modelos permiten
ofrecer respuestas contextualizadas, realizar tutorías personalizadas y asistir
en la construcción de conocimiento mediante diálogo interactivo. Su potencial
...
modelos de lenguaje de gran escala (LLMs) con motores de recuperación de
...
morales basados en evidencia. RAG amplía las capacidades de tutoría digital al
integrar conocimiento externo con generación de lenguaje natural
...
\begin{enumerate}
    \item \textbf{Fase de indexación:} Los documentos fuente se procesan mediante:
          ...
          implementación del sistema. En el artículo de lewis et al, la Figura
          \ref{fig:flujo-rag-teoria} muestra cómo este proceso se implementa mediante la
          combinación de \textbf{\guillemetleft{}memoria paramétrica\guillemetright{}}
          (el LLM utilizado para recibir preguntas y generar respuestas) y la
          ...
          La figura \ref{fig:flujo-rag-teoria} iustra la arquitectura general de un
    ...
    \item \textbf{Fase de inferencia:} Al recibir una consulta:
          \begin{itemize}
              ...
              \item El LLM genera una respuesta basándose exclusivamente en el contexto dado o
                    ...
                    et al, se menciona la diferencia de utilizar \textbf{RAG \textit{sequence}} (el
                    modelo selecciona un único documento sobre el cual basará su respuesta) frente
                    a \textbf{RAG \textit{token}} (el modelo genera la respuesta por pasos,
                    seleccionando la fuente a utilizar para cada token independiente, lo que
...
\subsection{Representaciones numéricas (\textit{embeddings}) y representación semántica del texto} Los \textit{embeddings} son
...
\subsubsection{Componentes de una petición efectiva} Al construir una petición que será recibida por un modelo de lenguaje de gran
escala (\textit{Large Language Model} o LLM), se deben tomar en cuenta varios
componentes que, en conjunto, serán los que definan la interacción entre el
modelo y el usuario. El proceso de ingeniería de peticiones consiste en
optimizar el lenguaje utilizado en la instrucción, enfocado en guiar el
comportamiento de los modelos de lenguaje y obtener el mejor desempeño posible.
Esto implica entender que los modelos procesan las instrucciones como programas
en lenguaje natural, por lo que la precisión y coherencia de las respuestas se
ven afectadas directamente por la estructura y redacción de cada parte de la
petición. \cite{schulhoff2025promptreportsystematicsurvey}
\begin{itemize}
    ...
    \item \textbf{Información Adicional:} Comunmente identificado también como el \textbf{contexto} de la petición. Se refiere a todo el contenido adicional que la IA debe tomar en cuenta para generar la respuesta, ya sea interacciones previas, o bien, el contenido que delimite la respuesta final (por ejemplo, a través de un sistema de generación aumentada por recuperación, o RAG).
...
\begin{itemize}
    \item \textbf{\textit{Chain-of-thought prompting} (Petición de Cadena de Pensamiento):} Solicita al modelo el razonamiento que utilizó para responder la pregunta, se exige el paso a paso de cómo llegó hasta la respuesta brindada.
...
En el contexto de sistemas RAG educativos, la \textit{faithfulness} o fidelidad
evalúa si la respuesta se limita estrictamente a la información contenida en
los documentos recuperados, evitando alucinaciones o afirmaciones no
sustentadas por el corpus proporcionado. \cite{es-etal-2024-ragas}
...
recuperación aumentada por generación (RAG), sistemas de recomendación,
...
\subsubsection{Servicio de Almacenamiento Simple (\textit{Simple Storage Service} o S3)} S3 es un servicio de almacenamiento de objetos altamente escalable,
...
mediante mecanismos de enlace de datos (\textit{dat"a binding}), lo que permite
...
vida de cada destino, la pila trasera, las transiciones y la integración con el
controlador de navegación (\textit{NavController}). En esencia, sirve como el
punto central que conecta la interfaz de usuario con la lógica de navegación
declarada, permitiendo manejar flujos complejos sin necesidad de administrar
manualmente transacciones de fragmentos. \cite{navhostfragment_androidx}
...
las transiciones correspondientes entre destinos. Además, administra la pila
trasera, permite navegar hacia destinos específicos, manejar argumentos tipados
(\textit{SafeArgs}) y coordinar animaciones o comportamientos especiales
definidos en la navegación. \cite{android_navcontroller}
...
\subsubsection{Parcelable}
Es una interfaz específica de Android diseñada para serializar objetos de
manera más rápida y eficiente que \textit{Serializable}. Permite descomponer un
...
\subsubsection{Lenguaje de Marcado Extensible (\textit{Extensible Markup Language})}
...
Retrofit es una librería de cliente HTTP (siglas en inglés para Protocolo de
Transferencia de HiperTexto) para Android y Java que permite consumir
\textit{APIs REST} (siglas en inglés para Transferencia de Estado Representacional) de forma sencilla. Soporta conversión transparente de JSON
...
HTTP, revisar respuestas, automatizar pruebas, organizar colecciones de puntos
de conexión y colaborar en equipos de desarrollo. Permite trabajar con
distintos métodos HTTP, autenticación, variables de entorno y generación
automática de documentación. \cite{postman_tool}
...
configuración. \cite{android\_viewmodelscope}
...
Modelos de Vista, Actividades o servicios. \cite{kotlin\_coroutinescope}
...
El término API se refiere a las siglas en inglés para \textbf{Interfaces de
    Programación de Aplicaciones}, las cuales permiten que dos sistemas de software
